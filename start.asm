...
;
;	 Adapted from template for use as a bootloader running in RAM.
;
;====================================================================
; 1  Copyright
;====================================================================
;  F2MC-16FX Family Template Project V01L08
;====================================================================
;/*******************************************************************************
...
; 5.13  Declaration of RAMCODE section and labels
; 5.14  Declaration of sections containing other sections description
; 5.15  Stack area and stack top definition/declaration
; 5.16  Direct page register dummy label definition
;
; 6  Start-Up Code
...
#set      STACK_USR_SIZE 768      ; <<< byte size of User stack
...
#set      CPU_24MHZ_CLKP1_16MHZ_CLKP2_16MHZ          0x5
#set      CLOCK_SPEED     CPU_32MHZ_CLKP1_32MHZ_CLKP2_16MHZ  ; <<< set clock speeds
...
#set      TBR_INIT_VALUE         0x0070   ; <<< define TBR value (address bits [23:8])
...
; #if OCDS_DEBUG_PIN_MODE == OCDS_DEBUG_PIN_SOFTGPIO
          ...
          ; .DATA.B 0xE4
          ; .DATA.B 0x36
          ; .RES.B 120                                ; update V01L08
; #elif OCDS_DEBUG_PIN_MODE == OCDS_DEBUG_PIN_GPIO
          ; .DATAB.B 18, 0x00                         ; update V01L08
          ; .RES.B 120                                ; update V01L08
; #else
          ; .DATAB.B 18, 0xFF                         ; update V01L08
          ; .RES.B 120                                ; update V01L08
; #endif
...
; params
	; The following NOPs provide area for passing in parameters
; _EX_OSC_MHZ
	NOP
; _CKSSR
	NOP
...
; _BGR
	NOP
	NOP
	; unused space for more parameters
	NOP
	NOP
	NOP
	NOP
...
; #else
  ...
  ; #elif CLOCK_SPEED == CPU_24MHZ_CLKP1_16MHZ_CLKP2_16MHZ
    ; ; Set PLL configuration depending on used crystal frequency
    ; #if CRYSTAL == FREQ_4MHZ
      ; MOVW PLLCR, #0x000B ; VMS[2:0]="000"(div2), PMS[4:0]="1011"(div12)
    ; #elif CRYSTAL == FREQ_8MHZ
      ; MOVW PLLCR, #0x0005 ; VMS[2:0]="000"(div2), PMS[4:0]="0101"(div6)
    ; #else
        ; #error Wrong CRYSTAL setting
    ; #endif
    ; ; set clock dividers for the current clock setting
    ; MOVW CKFCR, #0x2211 ; CLKP2="0010" (div3), CLKP1="0010" (div3), CLKB="0001" (div2), RCFS="0001"
...
; Clock setup for CPU_32MHZ_CLKP1_32MHZ_CLKP2_16MHZ
		; MOV  CKSSR, #(0x38 | MC_STAB_TIME)  ; set clock stabilization time
		; MOVW PLLCR, #0x0003 ; VMS[2:0]="000"(div2),PMS[4:0]="00011"(div4)
		; ; MOVW PLLCR, #0x0005 ; VMS[2:0]="000"(div2),PMS[4:0]="00101"(div6), This gets you a 48MHz CLKMC and CLKP1
		; MOVW CKFCR, #0x1001 ; CLKP2="0001"(div2),CLKP1="0000"(div1),CLKB="0000"(div1),RCFS="0001"
		; MOV  CKSR,  #0xFA ; change CLKS1 and CLKS2 to PLL clock(reamin SCE=1)
		
		; Below is a failed attempt to set these registers with the data being passed in.
		; MOVW CKSSR, #_CKSSR
		; MOVW PLLCR, #_PLLCR
		; MOVW CKFCR, #_CKFCR
		; MOV CKSR, #0xFA
...
;====================================================================
; 6.5   Set ROM mirror configuration
;====================================================================
...
; #  elif (CLOCK_SPEED == CPU_16MHZ_CLKP1_16MHZ_CLKP2_16MHZ) || \
        ...
        ; (CLOCK_SPEED == CPU_24MHZ_CLKP1_16MHZ_CLKP2_16MHZ)
; no_PLL_yet:
